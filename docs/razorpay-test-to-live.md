...
**Status as of 2026-09-19: the integration is built, tested and restored; it is
not switched on.** It was removed from the app in September and put back on
request; the payments table already exists in the live database, so the only
migration outstanding is `016_payment_ledger_integrity.sql`. Entry fees are collected end to end -- order creation, signature
...
Paste **`backend/db/migrations/APPLY_PENDING.sql`** into the Supabase SQL
editor — it carries 013, 014, 015 and 016 in the order they must be applied.

`015_payments.sql` creates the ledger. `016_payment_ledger_integrity.sql` is
the one that keeps it honest, and is not optional:

- a partial unique index so two payments cannot both settle against one entry
  (the application check is a read-then-write, and on serverless there is no
  process-local lock behind it);
- BEFORE DELETE triggers so deleting a tournament or a registration cannot
  destroy the record of money actually taken — both foreign keys are
  `ON DELETE CASCADE`, and the ledger is the only thing on our side linking a
  Razorpay payment to the entry it paid for;
- explicit grants, because 015 relied on Supabase's default privileges, which
  include TRUNCATE for `anon` and `authenticated` and which RLS does not
  restrain.

Neither is visible to `/api/health` beyond the table probe, so read the
`RAISE NOTICE` output in the SQL editor.
...
It also **refuses to open an order at all until `RAZORPAY_WEBHOOK_SECRET` is
set.** The browser callback is the happy path and the webhook is the only other
way a completed payment reaches this server, so taking money without one risks
a player paying and nothing on this side ever hearing about it. Orders answer
503 until the webhook exists.
...
- [ ] `APPLY_PENDING.sql` pasted (013, 014, 015, 016) and every `RAISE NOTICE`
      read; `/api/health` shows nothing pending
...
- [ ] `CORS_ORIGINS` covers the production origin; `ENV` is not `development`.
      `/api/health` reports `"cors": {"mode": "restricted"}` — `open` means ENV
      was not set, `blocking-all` means CORS_ORIGINS was not
